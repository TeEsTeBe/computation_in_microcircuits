...
from experiments.utils import connection_utils, compatability
...
class AmorphousCircuit(Microcircuit):
    ...
    def connect_net(self, print_connections=False):
        ...
        for (src_pop, trg_w_dict), (_, trg_probs) in zip(self.psp_amp_from_to.items(), self.probabilities_from_to.items()):
            ...
            for (trg_pop, weight), (_, probability) in tqdm(zip(trg_w_dict.items(), trg_probs.items()),
                ...
                if probability > 0.:
                    ...
                    mean_weight = connection_utils.calc_synaptic_weight_based_on_matlab_code(
                        psp_amp=weight,
                    ...
                    weights = connection_utils.get_conn_parameter_distribution(mean=mean_weight, part_std=0.7, size=n_connections)
                    u_values = connection_utils.get_conn_parameter_distribution(mean=syn_dict_template['U'], part_std=0.5, size=n_connections, maximum=1.)
                    delays = connection_utils.get_conn_parameter_distribution(mean=syn_dict_template['delay'], part_std=0.1, size=n_connections)
                    tau_recs = connection_utils.get_conn_parameter_distribution(mean=syn_dict_template['tau_rec'], part_std=0.5, size=n_connections)
                    tau_facs = connection_utils.get_conn_parameter_distribution(mean=syn_dict_template['tau_fac'], part_std=0.5, size=n_connections)

                    for i in range(n_connections):
                        syn_dict = syn_dict_template.copy()
                        syn_dict[compatability.syn_model_naming] = syn_models[i]
        ...
        print(f'Syn dict calculation time: {syn_calc_time} seconds ({round(syn_calc_time / 60, 2)} minutes; {round(syn_calc_time / 60 / 60, 4)} hours)')

        print('Connecting neurons ...')
        conn_start = time.time()
        self.connect_neurons_randomly_new(syn_dicts=syn_dicts)
        conn_time = time.time() - conn_start
        print(f'Connection time: {conn_time} seconds ({round(conn_time / 60, 2)} minutes; {round(conn_time / 60 / 60, 4)} hours)')
    ...
    def connect_neurons_randomly_new(self, syn_dicts):
        ...
        for syn_type, syn_parameter_list in tqdm(syn_dicts.items(), desc=f'connect populations', total=len(syn_dicts.keys()), disable=True):
