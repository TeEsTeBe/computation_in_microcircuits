...
class DegreeControlledCircuit(Microcircuit):
    ...
    def connect_net(self, print_connections=False):
        ...
        target_neurons = []
        synaptic_parameters = {
            compatability.syn_model_naming: [],
            'weight': [],
            'U': [],
            'u': [],
            'tau_rec': [],
        ...
        for (src_pop, trg_w_dict), (_, trg_probs) in zip(self.psp_amp_from_to.items(), self.probabilities_from_to.items()):
            for (trg_pop, weight), (_, probability) in zip(trg_w_dict.items(), trg_probs.items()):
                ...
                if probability > 0.:
                    ...
                    synaptic_parameters[compatability.syn_model_naming].extend(['tsodyks2_synapse' for _ in range(n_connections)])
                    ...
                    u_values = connection_utils.get_conn_parameter_distribution(mean=syn_dict['U'], part_std=0.5, size=n_connections, maximum=1.)
                    synaptic_parameters['U'].extend(u_values)
                    synaptic_parameters['u'].extend(u_values)
                    synaptic_parameters['delay'].extend(
                        connection_utils.get_conn_parameter_distribution(mean=syn_dict['delay'], part_std=0.1, size=n_connections)
                    )
                    synaptic_parameters['tau_rec'].extend(
                        connection_utils.get_conn_parameter_distribution(mean=syn_dict['tau_rec'], part_std=0.5, size=n_connections)
                    )
                    synaptic_parameters['tau_fac'].extend(
                        connection_utils.get_conn_parameter_distribution(mean=syn_dict['tau_fac'], part_std=0.5, size=n_connections)
                    )
