...
from experiments.utils import connection_utils, general_utils, compatability
    ...
    def connect_net(self, print_connections=False):
        ...
        for source_id, connection_map in conns_from_to.items():
            ...
            for target_neuron, target_pop in zip(connection_map['target_neurons'], connection_map['target_pops']):
                ...
                syn_dict['weight'] = connection_utils.calc_synaptic_weight_based_on_matlab_code(syn_weight, self.S_rw, source_pop[-3:], self.neuron_pars[source_pop]['g_L'])
                connection_utils.connect_population_pair(source_neuron, target_neuron, syn_dict=syn_dict, conn_dict='one_to_one')
