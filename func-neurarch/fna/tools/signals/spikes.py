...
class SpikeTrain(object):
    ...
    def isi_entropy(self, n_bins=100):
        ...
        """
        isi = self.isi()
        if not tools.utils.operations.empty(self.isi()):
            log_isi = np.log(isi)
    ...
    def frequency_spectrum(self, time_bin):
        ...
        freq_spect = metrics.simple_frequency_spectrum(hist)
    ...
    def exponential_filter(self, dt, tau, start=None, stop=None):
        ...
        if not tools.utils.operations.empty(spike_times):
            states, time_vec = signals.shotnoise_fromspikes(self, 1., tau, dt, t_start=start, t_stop=stop, array=True)
            return states
    ...
    def spikes_to_states_binary(self, dt, start=None, stop=None):
        ...
        if not tools.utils.operations.empty(spike_times):
            ...
            for x in spike_times:
                if not tools.utils.operations.empty(np.where(round(x, 1) == np.round(time_vec, 1))):
    ...
    def raster_plot(self, ax=None, t_start=None, t_stop=None, save=False, display=True):
        ...
        if t_stop is None:
            t_stop = self.t_stop
        viz.plotting.plot_single_raster(self.spike_times, ax, t_start, t_stop, save=save, display=display)
    ...
    def plot_isi_distribution(self, ax=None, save=False, display=True, **kwargs):
        ...
        :return:
        """
        viz.plotting.plot_isis(self.isi(), ax, save=save, display=display, **kwargs)
...
class SpikeList(object):
    ...
    def pairwise_cc(self, nb_pairs, pairs_generator=None, time_bin=1., average=True):
        ...
        for idx in range(N):
            ...
            if not average:
                results[idx, :] = metrics.ccf(hist_1, hist_2)
            else:
                results += metrics.ccf(hist_1, hist_2)
        if not average:
            return results
        else:
            return results / N
    ...
    def filter_spiketrains(self, dt, tau, start=None, stop=None, N=None, display=True):
        ...
        for idx, nn in enumerate(id_list):
            ...
            if display:
                viz.progress_bar(float(idx) / float(len(id_list)))
