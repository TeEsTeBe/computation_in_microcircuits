...
from fna.tasks.symbolic import SymbolicSequencer, NonAdjacentDependencies
...
def test_variable_durations():
    ...
    random_vector = VectorEmbeddings(vocabulary=sequencer.tokens).binary_codeword(dim=100, density=1.)
    enc = random_vector

    # enc.plot_sample(sample_sequences[0])
    # a) continuous signal
    signal_pars = {
        'duration': [250., 100., 150.],
        'amplitude': [1., 10., 20.],
        'kernel': ('box', {}),
        'dt': 0.1 # dt
    }
    sig = enc.unfold(to_signal=True, **signal_pars)
    assert sig.stimulus_set['0'][0].signal_length == 2500
    assert np.isclose(sig.stimulus_set['2'][0].analog_signals[0].signal.max(), 20.)
